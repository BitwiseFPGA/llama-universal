...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    ImRect total_bb(pos, pos + ImVec2(w, label_size.y + style.FramePadding.y * 2.0f));
    ImGui::ItemSize(total_bb, style.FramePadding.y);
    if (!ImGui::ItemAdd(total_bb, id))
        return false;

    // --- CHECKBOX POSITION (right aligned) ---
    ImVec2 check_pos = ImVec2(total_bb.Max.x - square_sz, pos.y);
    ImRect check_bb(check_pos, check_pos + ImVec2(square_sz, square_sz));

    // --- LABEL POSITION (left aligned) ---
    ImVec2 label_pos = ImVec2(pos.x, pos.y + style.FramePadding.y);

    // --- BUTTON BEHAVIOR (click if clicked anywhere in total_bb) ---
    bool hovered, held;
    ...
    if (*v)
    {
        ...
        ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 4.0f)),
            ImMax(1.0f, IM_FLOOR(square_sz / 4.0f)));
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, ImGuiComboFlags flags)
{
    ...
    // --- FULL WIDTH LAYOUT ---
    const float w_full = GetContentRegionAvail().x;
    const float combo_width = GetFrameHeight() * 8.0f; // Fixed combo width to match slider

    const ImVec2 pos = window->DC.CursorPos;

    // Combo box position (right aligned)
    const ImVec2 combo_pos = ImVec2(pos.x + w_full - combo_width, pos.y);
    const ImRect bb(combo_pos, combo_pos + ImVec2(combo_width, label_size.y + style.FramePadding.y * 2.0f));
    const ImRect total_bb(pos, pos + ImVec2(w_full, label_size.y + style.FramePadding.y * 2.0f));
    ...
    if (!(flags & ImGuiComboFlags_NoArrowButton))
    {
        ...
        ImU32 text_col = GetColorU32(ImGuiCol_Text);
        window->DrawList->AddRectFilled(ImVec2(value_x2, bb.Min.y), bb.Max, bg_col, style.FrameRounding, (combo_width <= arrow_size) ? ImDrawFlags_RoundCornersAll : ImDrawFlags_RoundCornersRight);
    ...
    // --- DRAW LABEL ON LEFT ---
    if (label_size.x > 0)
        RenderText(ImVec2(pos.x, bb.Min.y + style.FramePadding.y), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const float slider_width = GetFrameHeight() * 8.0f; // Width of the slider itself
    ...
    const ImVec2 slider_pos = ImVec2(total_bb.Max.x - slider_width, pos.y);
    const ImRect frame_bb(slider_pos, slider_pos + ImVec2(slider_width, label_size.y + style.FramePadding.y * 2.0f));

    // --- LABEL POSITION (left aligned) ---
    const ImVec2 label_pos = ImVec2(pos.x, pos.y + style.FramePadding.y);
    ...
    // Tabbing or CTRL-clicking on Slider turns it into an InputText
    const bool temp_input_allowed = (flags & ImGuiSliderFlags_NoInput) == 0;
    ...
    if (grab_bb.Max.x > frame_bb.Min.x + 2.f) // only draw if moved at least 1 pixel
    {
        ImVec2 filled_min = ImVec2(frame_bb.Min.x, grab_bb.Min.y);
        ImVec2 filled_max = ImVec2(grab_bb.Max.x, grab_bb.Max.y);
        window->DrawList->AddRectFilled(filled_min, filled_max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.FrameRounding);
    }
